...
const NumberBoxButtonComponent = (props) => {
  const componentState = props.isActive ? 'active' : 'inactive';
  const { type } = props;
  const increment = type == 'plus' ? 1 : -1;
  return (
    <button
      className={`btn rbc-btn rbc-numberbox-btn rbc-numberbox-btn-${componentState}`}
      onClick={() => props.handleChange(increment)}>
...
const NumberComponent = (props) => {
  ...
  const value = props.value ? props.value : this.state.currentValue;
  const isPlusActive = max ? value < max : true;
  const isMinusActive = min ? value > min : true;
...
class NumberBox extends Component {
  ...
  // use this only if want to create actuators
	...
	createChannel() {
		const depends = this.props.depends ? this.props.depends : {};
