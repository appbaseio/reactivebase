...
export class MapStyles extends Component {
	...
	componentDidMount() {
		...
		if(this.props.defaultSelected) {
			AppbaseSensorHelper.mapStyles.forEach((style, index) => {
		...
		this.setState({
			items: AppbaseSensorHelper.mapStyles,
	...
	themeChanged(isExecute=false) {
		let style = AppbaseSensorHelper.mapStyles[this.state.selectedValue].value;
	...
	render() {
		...
		return (
			<div className="input-field col mapStyles pull-right right">
