...
var Pagination = function (_Component) {
	...
	function Pagination(props) {
		...
		var page = parseInt(helper.URLParams.get("page"));
		_this.state = {
	...
	Pagination.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
		if (nextProps.pageURLParams && nextProps.pageURLParams !== this.props.pageURLParams) {
			var page = parseInt(helper.URLParams.get("page")) || 1;
			this.setState({
				currentValue: page,
				maxPageNumber: page
			});
	...
	Pagination.prototype.listenGlobal = function listenGlobal() {
		...
		this.globalListener = manager.emitter.addListener("global", function (res) {
			if (res.react && Object.keys(res.react).indexOf(_this2.props.componentId) > -1) {
				...
				var page = parseInt(helper.URLParams.get("page"));
				if (_this2.props.pageURLParams && page && currentPage !== page) {
					_this2.setState({
						totalHits: totalHits,
						size: size,
