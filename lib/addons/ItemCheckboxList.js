...
var ItemCheckboxList = function (_Component) {
		...
		if (Array.isArray(this.props.defaultSelected)) {
			if (this.props.defaultSelected.indexOf(this.props.selectAllLabel) >= 0) {
				...
				this.props.items.forEach(function (item) {
					if (Array.isArray(_this2.props.defaultSelected) && _this2.props.defaultSelected.indexOf(item.key) >= 0) {
						items.push(item.key);
					}
				});
				this.setState({
	...
	ItemCheckboxList.prototype.removeItem = function removeItem(value) {
		...
		this.setState({
			...
			_this5.props.onRemove(_this5.state.selectedItems);
		});
	};
	...
	ItemCheckboxList.prototype.handleListClickAll = function handleListClickAll(value, selectedStatus) {
		...
		if (selectedStatus) {
			...
			this.setState({
				...
				_this6.props.onSelectAll(value);
			});
		} else {
			this.setState({
				selectedItems: []
			}, function () {
				_this6.props.onSelect(null);
			});
		}
	};
	...
	ItemCheckboxList.prototype.render = function render() {
		...
		items.forEach(function (item, index) {
			...
			ListItemsArray.push(React.createElement(ListItem, {
				...
				countField: _this7.props.showCount,
				handleClick: _this7.handleListClick,
				visible: visibleFlag,
				showCheckbox: _this7.props.showCheckbox,
				status: _this7.state.selectedItems.indexOf(item.key) >= 0,
				ref: function ref(listitem) {
					var currentItemRef = "ref" + item.keyRef;
					_this7.refStore[currentItemRef] = listitem;
				}
			}));
		});
		...
		if (this.props.selectAllLabel && items && items.length) {
			ListItemsArray.unshift(React.createElement(ListItem, {
				...
				ref: function ref(listitem) {
					_this7.refStore.refselectall = listitem;
				}
			}));
		}
