...
var MultiRange = function (_Component) {
	...
	MultiRange.prototype.handleChange = function handleChange(record) {
		...
		if (record) {
			...
			if (setTrue) {
				...
				records.forEach(function (item) {
					if (_this3.props.data.some(function (label) {
		...
		var execQuery = function execQuery() {
			...
			helper.URLParams.update(_this3.props.componentId, _this3.getSelectedLabels(selected), _this3.props.URLParams);
	...
	MultiRange.prototype.renderButtons = function renderButtons() {
		...
		if (this.props.data) {
			buttons = this.props.data.map(function (record) {
				var cx = classNames({
					"rbc-checkbox-active": _this4.props.showCheckbox,
					"rbc-checkbox-inactive": !_this4.props.showCheckbox,
