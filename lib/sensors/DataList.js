	...
	DataList.prototype.reset = function reset() {
		...
		var execQuery = function execQuery() {
			if (_this4.props.onValueChange) {
				_this4.props.onValueChange(null);
			}

			helper.URLParams.update(_this4.props.componentId, null, _this4.props.URLParams);
	...
	DataList.prototype.executeQuery = function executeQuery(value) {
		...
		var execQuery = function execQuery() {
			if (_this7.props.onValueChange) {
				if (_this7.state.selectAll) {
					_this7.props.onValueChange(_this7.props.data);
				} else {
					_this7.props.onValueChange(value);
				}
			}
					...
					return React.createElement(
						"div",
						{
							...
							onClick: function onClick() {
								_this8.handleCheckboxChange(record);
							} },
		...
		if (this.props.multipleSelect) {
			if (this.state.selectAll) {
				...
				this.setState({
					...
					_this10.executeQuery(_this10.props.selectAllLabel);
				});
			}
		} else {
			this.setState({
