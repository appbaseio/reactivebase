	...
	DataList.prototype.executeQuery = function executeQuery(value) {
		...
		var execQuery = function execQuery() {
			if (_this6.props.onValueChange) {
				if (_this6.state.selectAll) {
					_this6.props.onValueChange(_this6.props.data);
				} else {
					_this6.props.onValueChange(value);
				}
			}

			var selectedValue = typeof value === "string" ? value.trim() ? value : null : value;
			helper.URLParams.update(_this6.props.componentId, selectedValue, _this6.props.URLParams);
			helper.selectedSensor.set(obj, true);
		};
		...
		if (this.props.multipleSelect) {
			if (this.state.selectAll) {
				...
				this.setState({
					...
					_this9.executeQuery(_this9.props.selectAllLabel);
				});
			}
		} else {
			this.setState({
