...
var RangeSlider = function (_Component) {
		...
		setTimeout(function () {
				...
				if (rem) {
					...
					setTimeout(function () {
						if (_this2.props.beforeValueChange) {
							_this2.props.beforeValueChange(values).then(function () {
								execQuery();
							}).catch(function (e) {
					...
					setTimeout(function () {
						if (_this2.props.beforeValueChange) {
							_this2.props.beforeValueChange(_values).then(function () {
								execQuery();
							}).catch(function (e) {
								console.warn(_this2.props.componentId + " - beforeValueChange rejected the promise with", e);
							});
			...
			if (nextProps.range.start !== _this2.state.startThreshold || nextProps.range.end !== _this2.state.endThreshold) {
				if (nextProps.range.start <= defaultValue.start && nextProps.range.end >= defaultValue.end) {
					...
					if (_this2.props.beforeValueChange) {
						_this2.props.beforeValueChange(_values2).then(function () {
							execQuery();
						}).catch(function (e) {
							console.warn(_this2.props.componentId + " - beforeValueChange rejected the promise with", e);
						});
			...
			if (nextProps.stepValue !== _this2.props.stepValue) {
				...
				if (_rem) {
					...
					if (_this2.props.onValueChange) {
						_this2.props.onValueChange(_obj4.value);
					}
					helper.URLParams.update(_this2.props.componentId, _this2.setURLParam(_obj4.value), _this2.props.URLParams);
					helper.selectedSensor.set(_obj4, true);
				}
			}
		}, 300);
	};
	...
	RangeSlider.prototype.handleResults = function handleResults(textVal, value) {
		...
		var execQuery = function execQuery() {
			...
			helper.URLParams.update(_this6.props.componentId, _this6.setURLParam(obj.value), _this6.props.URLParams);
			helper.selectedSensor.set(obj, true);
		};

		if (this.props.beforeValueChange) {
			this.props.beforeValueChange(values).then(function () {
				execQuery();
			}).catch(function (e) {
