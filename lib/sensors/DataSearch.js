...
var DataSearch = function (_Component) {
	...
	DataSearch.prototype.highlightQuery = function highlightQuery(props) {
		var fields = {};
		var highlightField = props.highlightField ? props.highlightField : props.appbaseField;
	...
	DataSearch.prototype.setData = function setData(data) {
		...
		data.hits.hits.map(function (hit) {
			if (_this4.fieldType === "string") {
				var tempField = _this4.getValue(_this4.props.appbaseField.trim(), hit._source);
				options.push({ value: tempField, label: tempField });
			} else if (_this4.fieldType === "object") {
				_this4.props.appbaseField.map(function (field) {
					var tempField = _this4.getValue(field, hit._source);
					if (tempField) {
						options.push({ value: tempField, label: tempField });
					}
				});
	...
	DataSearch.prototype.shouldQuery = function shouldQuery(value, appbaseFields) {
		...
		var fields = appbaseFields.map(function (field, index) {
			return "" + field + (Array.isArray(_this5.props.searchWeight) && _this5.props.searchWeight[index] ? "^" + _this5.props.searchWeight[index] : "");
		});
	...
	DataSearch.prototype.handleSearch = function handleSearch(currentValue) {
		...
		var execQuery = function execQuery() {
			...
			helper.URLParams.update(_this7.props.componentId, value, _this7.props.URLParams);
			helper.selectedSensor.set(obj, true);
			_this7.setState({
				currentValue: value
			});
	...
	DataSearch.prototype.handleInputChange = function handleInputChange(event) {
		...
		var execQuery = function execQuery() {
			...
			helper.URLParams.update(_this8.props.componentId, inputVal, _this8.props.URLParams);
