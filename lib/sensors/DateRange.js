...
var DateRange = function (_Component) {
	...
	DateRange.prototype.componentWillReceiveProps = function componentWillReceiveProps() {
		this.checkDefault();
	};
	...
	DateRange.prototype.setQueryInfo = function setQueryInfo() {
		var obj = {
	...
	DateRange.prototype.isDateChange = function isDateChange() {
		...
		var isChanged = function isChanged(defaultSelected) {
			if (moment(_this3.startDate).format(helper.dateFormat[_this3.props.queryFormat]) !== moment(defaultSelected.start).format(helper.dateFormat[_this3.props.queryFormat]) && moment(_this3.endDate).format(helper.dateFormat[_this3.props.queryFormat]) !== moment(defaultSelected.end).format(helper.dateFormat[_this3.props.queryFormat])) {
				_this3.startDate = defaultSelected.start;
				_this3.endDate = defaultSelected.end;
				flag = true;
			}
	...
	DateRange.prototype.generateQuery = function generateQuery(value) {
		...
		if (Array.isArray(this.props.appbaseField) && this.props.appbaseField.length === 2) {
			...
			query = {
				bool: {
					must: [{
						range: (_range = {}, _range[this.props.appbaseField[0]] = {
							lte: moment(value.startDate).format(helper.dateFormat[this.props.queryFormat])
						}, _range)
					}, {
						range: (_range2 = {}, _range2[this.props.appbaseField[1]] = {
			...
			query = {
				range: (_range3 = {}, _range3[this.props.appbaseField[0]] = {
			...
			query = {
				range: (_range4 = {}, _range4[this.props.appbaseField] = {
					gte: moment(value.startDate).format(helper.dateFormat[this.props.queryFormat]),
					lte: moment(value.endDate).format(helper.dateFormat[this.props.queryFormat])
				}, _range4)
			};
	...
	DateRange.prototype.setValue = function setValue(inputVal) {
		...
		var execQuery = function execQuery() {
			...
			helper.URLParams.update(_this4.props.componentId, _this4.urlFriendlyValue(inputVal), _this4.props.URLParams);
