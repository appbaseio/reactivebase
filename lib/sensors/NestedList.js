...
var NestedList = exports.NestedList = function (_Component) {
	...
	_createClass(NestedList, [{
		...
		value: function componentWillUpdate() {
			...
			setTimeout(function () {
				if (_this4.defaultSelected != _this4.props.defaultSelected) {
					_this4.defaultSelected = _this4.props.defaultSelected;
					var items = _this4.state.items;
					items = items.map(function (item) {
						item.key = item.key.toString();
						item.status = _this4.defaultSelected && _this4.defaultSelected.indexOf(item.key) > -1 ? true : false;
						return item;
					});
					_this4.setState({
						items: items,
		...
		value: function createSubChannel() {
			...
			// create a channel and listen the changes
			var subChannelObj = _ChannelManager.manager.create(this.context.appbaseRef, this.context.type, depends);
		...
		value: function addItemsToList(newItems, level) {
			...
			newItems = newItems.map(function (item) {
				item.key = item.key.toString();
				item.status = _this5.defaultSelected && _this5.defaultSelected.indexOf(item.key) > -1 ? true : false;
				return item;
			});
		...
		value: function renderItems(items, level) {
			...
			return items.map(function (item, index) {
				var cx = (0, _classnames2.default)({
					'rbc-item-active': item.key === _this6.state.selectedValues[level],
					'rbc-item-inactive': !(item.key === _this6.state.selectedValues[level])
				});
				return _react2.default.createElement(
					...
					_react2.default.createElement(
						...
						{ href: 'javascript:void(0);', className: 'rbc-list-item ' + cx, onClick: function onClick() {
								return _this6.onItemSelect(item.key, level);
							} },
						_react2.default.createElement(
							'span',
							null,
						...
						_this6.renderChevron(level)
					),
					_this6.renderList(item.key, level)
				);
			});
		}
	}, {
		...
		value: function renderList(key, level) {
			...
			if (key === this.state.selectedValues[level] && level === 0) {
				list = _react2.default.createElement(
					'ul',
					{ className: 'rbc-sub-nestedlist rbc-indent col s12 col-xs-12' },
		...
		value: function render() {
			...
			return _react2.default.createElement(
				'div',
				{ className: 'rbc rbc-nestedlist col s12 col-xs-12 card thumbnail ' + cx, style: this.props.defaultStyle },
				title,
				searchComponent,
				listComponent
			);
...
NestedList.propTypes = {
	...
	defaultQuery: _react2.default.PropTypes.func,
