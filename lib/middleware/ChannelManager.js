...
var channelManager = function () {
	...
	_createClass(channelManager, [{
			...
			if (validQuery) {
				(function () {
					...
					if (appbaseRef) {
						...
						searchQueryObj.type = _this.type == '*' ? '' : _this.type;
						...
						var streamQueryObj = queryObj;
						streamQueryObj.type = _this.type;
						_this.streamRef[channelId] = appbaseRef.searchStream(streamQueryObj).on('data', function (data) {
							var obj = {
								mode: 'stream',
								data: data,
								appliedQuery: queryObj
		...
		key: 'create',
		value: function create(appbaseRef, type, depends) {
			var _this2 = this;
